<script>
  ...
  if (browser) authParams.push(
    "redirect_uri=" + encodeURIComponent(window.location.origin + window.location.pathname)
  );

  let lastError = "", needsRequest = false;
  ...
  onMount(() => {
    ...
    const query = querySource.slice(1).split("&").reduce(
      (ac, cv) => {
    ...
    try {
      ...
      if (query.error) {
        throw new Error(decodeURIComponent(query.error_description || query.error).replace(/\+/g," "));
      }
...
<h5 class="text-warning mt-2">
  If you are streaming, please do not show this window.
</h5>

{#if needsRequest}
<a role="button" class="btn btn-twitch shadow my-4 fs-4"
